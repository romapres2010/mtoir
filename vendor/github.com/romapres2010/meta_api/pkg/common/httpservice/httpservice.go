...
import (
    ...
    bytespool "github.com/romapres2010/meta_api/pkg/common/bytespool"
    ...
    _jwt "github.com/romapres2010/meta_api/pkg/common/jwt"
...
func (s *Service) Process(ignoreBufPool bool, method string, w http.ResponseWriter, r *http.Request, fn func(ctx context.Context, requestBuf []byte, buf []byte) ([]byte, Header, int, error)) (myerr error) {
    ...
    // Добавим метрики
    _metrics.IncHTTPRequestsDurationVec(r.RequestURI, r.Method, time.Now().Sub(tic))
    ...
    if w != nil && r != nil && err != nil {
        ...
        if s.cfg.HTTPErrorLogBody {
            ...
            if !isMyErr {
                ...
                errorResponse = &ErrorResponse{Error: _err.WithCauseTyped(_err.ERR_COMMON_ERROR, _err.ERR_UNDEFINED_ID, err, err.Error())}
            } else {
                myerr.Trace = ""
                myerr.Args = ""
