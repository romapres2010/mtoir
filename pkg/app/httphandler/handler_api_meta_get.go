...
func (s *Service) ApiGetEntityMetaHandler(w http.ResponseWriter, r *http.Request) {
	...
	_ = s.httpService.Process(false, "GET", w, r, func(ctx context.Context, requestBuf []byte, buf []byte) ([]byte, _http.Header, int, error) {
		...
		// вызываем сервис
		exists, responseBuf, err := s.apiService.GetEntityMeta(ctx, entityName, format)
		if err != nil {
			if !exists {
		...
		case "proto":
			header[_http.HEADER_CONTENT_TYPE] = _http.HEADER_CONTENT_TYPE_PLAIN_UTF8
		default:
			return requestBuf, nil, http.StatusBadRequest, _err.NewTyped(_err.ERR_INCORRECT_CALL_ERROR, _err.ERR_UNDEFINED_ID, "Allowed only format='json', 'yaml', 'xml', 'proto'", format).PrintfError()
		}
