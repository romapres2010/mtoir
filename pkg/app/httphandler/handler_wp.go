...
import (
	...
	_wp "github.com/romapres2010/meta_api/pkg/common/workerpool"
...
func calculateFactorial(ctx context.Context, wpService *_wpservice.Service, requestID uint64, wpFactorialReqResp *WpFactorialReqResp, wpTipe string, tasks []*_wp.Task) (err error) {
	if wpTipe == "bg" {
		...
		err = wpService.RunTasksGroupWG(requestID, tasks, "CalculateField - background")
		//err = wpService.RunTasksGroupCh(requestID, tasks, "CalculateField - background")
