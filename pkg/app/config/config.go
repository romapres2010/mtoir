...
import (
	...
	apidbstorage "github.com/romapres2010/meta_api/pkg/meta_api/storagedb"
...
func (cfg *Config) LoadYamlConfig(fileName string) error {
	if cfg != nil {
		...
		{ // подменить параметры из окружения и командной строки, приоритет за командной строкой
			...
			replaceCfgArgsEnv(
				"api_service.entity_config_file", &cfg.ApiServiceCfg.EntityConfigFile,
				"config_file", cfg.ArgsMetaEntityConfigFile,
				ENV_APP_META_ENTITY_CONFIG_FILE, cfg.EnvMetaEntityConfigFile)
